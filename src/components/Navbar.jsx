    ...
    <nav className={`${styles.paddingX} fixed top-0 inset-x-0 z-30 pt-4 pb-4`}>
      <div
        className={`max-w-7xl mx-auto flex items-center justify-between gap-4 px-4 sm:px-6 py-3 rounded-2xl transition-all duration-300 ${
          scrolled
            ? "bg-primary/80 backdrop-blur-xl border border-ink/10 shadow-lg shadow-black/20"
            : "bg-transparent"
        }`}
      >
        <a
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand to-accent p-[1px]">
            <div className="w-full h-full rounded-[11px] bg-primary flex items-center justify-center">
              <img src={om} alt="Osama Mumtaz" className="w-6 h-6 object-contain icon-invert" />
            </div>
          </div>
          <div className="hidden xs:block">
            <p className="text-ink font-medium text-[15px] leading-tight">Osama Mumtaz</p>
            ...
            <li key={nav.id}>
              <a
                ...
                className={`px-4 py-2 rounded-xl text-[14px] font-normal transition-colors ${
                  active === nav.title
                    ? "text-ink bg-ink/10"
                    : "text-secondary hover:text-ink hover:bg-ink/5"
                }`}
              >
                {nav.title}
              </a>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <img
              ...
              alt=""
              className="w-7 h-7 object-contain icon-invert"
            />
          </button>
        </div>
      </div>
      ...
      {toggle && (
        <div className="md:hidden max-w-7xl mx-auto mt-3 px-4 py-5 rounded-2xl bg-primary/95 backdrop-blur-xl border border-ink/10">
            ...
            {navLinks.map((nav) => (
              <li key={nav.id}>
                <a
                  ...
                  className="block px-4 py-3 rounded-xl text-secondary hover:text-ink hover:bg-ink/5 text-[15px] font-medium"
                >
                  {nav.title}
                </a>
              </li>
